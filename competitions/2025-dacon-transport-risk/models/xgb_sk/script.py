...
def align_to_model(X_df, model):
    # helper: try various places where feature names might be stored
    def _get_model_feature_names(m):
        # scikit-learn wrapper attribute (may be present)
        if hasattr(m, "feature_name_") and getattr(m, "feature_name_"):
            return list(m.feature_name_)
        # xgboost Booster inside sklearn wrapper
        try:
    ...
    # 누락 피처는 0으로 채우고, 초과 피처는 제거하면서 순서 맞춤
    for c in feat_names:
...
def main():
    ...
    # ---- 모델 로드 ----
    print("Load models...")
    model_A = joblib.load(os.path.join(MODEL_DIR, "xgb_A.pkl"))
    model_B = joblib.load(os.path.join(MODEL_DIR, "xgb_B.pkl"))
    print(" OK.")
    ...
    # debug: 모델이 기대하는 feature list와 실제 DataFrame의 columns를 비교해 출력
    def _dbg_check(X_df, model, label):
        model_feats = list(getattr(model, "feature_name_", []))
        try:
            if not model_feats and hasattr(model, "get_booster"):
                b = model.get_booster()
                model_feats = list(getattr(b, "feature_names", []) or [])
        except Exception:
            pass
        X_cols = list(X_df.columns)
        if not model_feats:
            print(f"[DEBUG] {label} model has no stored feature_name_, X has {len(X_cols)} columns: {X_cols[:10]}{'...' if len(X_cols)>10 else ''}")
            return
        if X_cols == model_feats:
            print(f"[DEBUG] {label} features aligned: {len(X_cols)} columns")
            return
        missing = [c for c in model_feats if c not in X_cols]
        extra   = [c for c in X_cols if c not in model_feats]
        print(f"[DEBUG] {label} model expects {len(model_feats)} features, X has {len(X_cols)} columns")
        if missing:
            print(f"[DEBUG] missing in X_{label}: {missing[:20]}{'...' if len(missing)>20 else ''}")
        if extra:
            print(f"[DEBUG] extra in X_{label}: {extra[:20]}{'...' if len(extra)>20 else ''}")

    # ---- 예측 ----
    print("Inference Model...")
    predA = model_A.predict_proba(XA)[:,1] if len(XA) else np.array([])
    predB = model_B.predict_proba(XB)[:,1] if len(XB) else np.array([])
    _dbg_check(XA, model_A, 'A')
    _dbg_check(XB, model_B, 'B')
